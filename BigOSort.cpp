...
const int MAX_MENU = 4; // # of menu options  {MODIFY WHEN NEEDED!!!}
const int MAX_LEVELS=300;   // Used by quicksort() for partitioning
/****************[ Global Variables ]****************/
char file_path[300] = ""; // file name concated later
  ...
  while (key != 0)
  {
    ...
    switch (key)
    {
      case 0:
	          ...
	          exit(EXIT_SUCCESS);
      case 1:
				// Load data into the array..if it fails, exit
				if ((loadData(fName)) == -1)
			  	return -1;
				// Start the timer
				exeTime(&begin,NULL);
				...
				break;
      case 2:
				// Load teh data into the array..if it fails, exit
				if ((loadData(fName)) == -1)
			  	return -1;
				// Start timer
				exeTime(&begin,NULL);
